...
db.create_all() # Line below only required once, when creating DB
...
def register():
    ...
    if user_form.validate_on_submit():
        ...
        password_hash = generate_password_hash(user_form.password.data, method='pbkdf2:sha256', salt_length=8)
        new_user = User(name=user_form.name.data, email=user_form.email.data, password=password_hash,)
        db.session.add(new_user)
        db.session.commit()
        login_user(new_user) # что бы авторизовать нового пользователя в приложении
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        new_comment = Comment(
            ...
            comment_author_id=current_user,
